...
**如何应用**：
- **必须在浏览器连续看多个完整周期**，并覆盖最长的次级动作，才能锁定
...
**典型案例**：拖拽 reaction 如果只是静态扭曲贴图，会看起来像死物。最简单的 reaction 也要保留符合状态语义的微动。

**如何应用**：
- 从呼吸、重心、头发、衣物、耳朵或环境反馈里选择适合该状态的微动
- 睡眠、闭眼、凝视或通知状态可以明确锁定眼睛，不机械添加眨眼
- “简单”指减少无关装饰和大动作，仍要让连续播放读得出状态
...
**正确做法**：按 `routes/svg/conventions/validation-runbook.md` 分层验证：结构、自包含、脚本可解析、真实嵌入方式、多个完整循环周期、目标端复验。
...
4. **查 transform-origin**：位置漂移经常来自错误的 origin
